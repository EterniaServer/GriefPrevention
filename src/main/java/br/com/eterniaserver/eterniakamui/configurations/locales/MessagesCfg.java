...
import br.com.eterniaserver.eterniakamui.CustomizableMessage;
...
public class MessagesCfg {
    ...
    public MessagesCfg(String[] messages) {
        ...
        this.addDefault(defaults, Messages.NoPvPInDay, "Você não pode entrar em combate durante o dia$8.", null);
        ...
        for (Messages messageID : Messages.values()) {
            ...
            messages[messageID.ordinal()] = messagesConfig.getString("Messages." + messageID.name() + ".Text", messageData.text);
            messagesConfig.set("Messages." + messageID.name() + ".Text", messages[messageID.ordinal()]);
            ...
            if (messageData.notes != null) {
                messageData.notes = messagesConfig.getString("Messages." + messageID.name() + ".Notes", messageData.notes);
                messagesConfig.set("Messages." + messageID.name() + ".Notes", messageData.notes);
            }
        }

        if (new File(Constants.DATA_LOCALE_FOLDER_PATH).mkdir()) {
            EterniaKamui.AddLogEntry("ERR: 00F7E9E731");
        }
